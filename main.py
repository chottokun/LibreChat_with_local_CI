...
RCE_DATA_DIR_INTERNAL = os.environ.get(
...
if not _raw_data_dir or "absolute/path/to/your/project/sessions" in _raw_data_dir:
    ...
    if _raw_data_dir:
        logger.info(
            "RCE_DATA_DIR is set to placeholder. Using default 'put_archive' mode."
        )
else:
...
if RCE_DATA_DIR_HOST:
    try:
        ...
        if not os.access(RCE_DATA_DIR_INTERNAL, os.W_OK):
            ...
            logger.warning(
                f"RCE_DATA_DIR is set to '{RCE_DATA_DIR_HOST}', but the internal path '{RCE_DATA_DIR_INTERNAL}' is not writable."
            )
            logger.warning(
                "Falling back to 'put_archive' mode (slower, but works without host mounting)."
            )
            logger.warning(
                "To fix this, ensure the host directory has correct permissions (e.g., sudo chown -R 1000:1000 <dir>)"
            )
            RCE_DATA_DIR_HOST = None
        else:
            logger.info(
                f"Volume mounting enabled: {RCE_DATA_DIR_HOST} -> {RCE_DATA_DIR_INTERNAL}"
            )
    except Exception as e:
        logger.warning(
            f"Failed to initialize shared volume: {e}. Falling back to 'put_archive' mode."
        )
        RCE_DATA_DIR_HOST = None
...
async def get_api_key(
    ...
    auth_key = (
        token.credentials if isinstance(token, HTTPAuthorizationCredentials) else None
    )
...
def _get_session_ids(sid: str) -> Tuple[str, str]:
    ...
    if isinstance(kernel_manager, MagicMock):
        ...
        real_session_id, nanoid_session = kernel_manager.get_or_create_session_mapping(
            sid
        )
...
class KernelManager:
    ...
    def __init__(self):
        ...
        self.pending_sessions = (
            set()
        )  # Track sessions currently in the process of starting
        ...
        self.file_id_map: Dict[
            str, Dict[str, str]
        ] = {}  # {nanoid_session_id: {nanoid_file_id: filename}}
        ...
        with self.lock:
            ...
            if not real_session_id:
                if s_sid in self.session_to_nanoid:
                    ...
                    logger.info(
                        "Mapped provided SID %s to new internal UUID %s",
                        s_sid,
                        real_session_id,
                    )
        ...
        with self.lock:
            ...
            if nanoid_session in self.file_id_map:
                if s_filename in self.file_id_map[nanoid_session]:
                    ...
                    real_filename = self.file_id_map[nanoid_session][
                        os.path.basename(s_filename)
                    ]
            ...
            if container_to_refresh:
                try:
                    ...
                    if container_to_refresh.status == "running":
                        with self.lock:
                            if session_id in self.active_kernels:
                                self.active_kernels[session_id]["last_accessed"] = (
                                    time.time()
                                )
                    ...
                    logger.exception(
                        "Error refreshing container for session %s", session_id
                    )
                    with self.lock:
                        self.active_kernels.pop(session_id, None)
                    # Fall through to create new
    ...
    def start_new_container(
        self, session_id: str, external_session_id: Optional[str] = None
    ):
        session_lock = self._get_session_lock(session_id)
        with session_lock:
            ...
            with self.lock:
                ...
                if (
                    len(self.active_kernels) + len(self.pending_sessions)
                    >= RCE_MAX_SESSIONS
                ):
                    logger.warning(
                        "Max sessions reached: %d (Active: %d, Pending: %d)",
                        RCE_MAX_SESSIONS,
                        len(self.active_kernels),
                        len(self.pending_sessions),
                    )
                    raise HTTPException(
                        status_code=503,
            ...
            try:
                ...
                if container:
                    try:
                        ...
                        logger.error(
                            "Failed to stop container %s after startup failure: %s",
                            container.id if hasattr(container, "id") else "unknown",
                            stop_err,
                        )
                raise HTTPException(
                    status_code=500,
                    detail="Failed to start sandbox. Please contact an administrator.",
                )
    ...
    def _get_container_config(self) -> Dict[str, Any]:
        ...
        if self._cached_config is None:
            ...
            cpu_limit_nano = int(
                os.environ.get("RCE_CPU_LIMIT", "500000000")
            )  # 0.5 CPU default
            network_enabled = (
                os.environ.get("RCE_NETWORK_ENABLED", "false").lower() == "true"
            )
    ...
    def recover_containers(self):
        ...
        try:
            containers = DOCKER_CLIENT.containers.list(
                all=True, filters={"label": f"managed_by={RCE_MANAGED_BY_VALUE}"}
            )
            with self.lock:
                    ...
                    if session_id and session_id not in self.active_kernels:
                        try:
                            ...
                            if external_session_id:
                                ...
                                logger.info(
                                    "Recovered session %s (external: %s) from container %s",
                                    session_id,
                                    external_session_id,
                                    container.id,
                                )
                                ...
                                logger.info(
                                    "Recovered session %s from container %s",
                                    session_id,
                                    container.id,
                                )
                        except Exception as e:
                            logger.error(
                                "Failed to recover container %s: %s", container.id, e
                            )
        except Exception as e:
    ...
    def _cleanup_single_session(self, session_id: str, data: Optional[Dict[str, Any]]):
        ...
        try:
            ...
            if RCE_DATA_DIR_INTERNAL:
                ...
                if not safe_sid:
                    logger.warning(
                        "Attempted to cleanup session with invalid ID: %s", session_id
                    )
                    return
    ...
    def upload_file(
        ...
        try:
            if RCE_DATA_DIR_HOST:
                ...
                logger.info(
                    "Uploaded file %s to volume (internal: %s) for session %s",
                    safe_filename,
                    session_dir,
                    safe_sid,
                )
                ...
                self.get_or_create_container(
                    safe_sid, external_session_id=external_session_id
                )
            else:
                container = self.get_or_create_container(
                    safe_sid, external_session_id=external_session_id
                ...
                logger.info(
                    "Uploaded file %s to session %s via put_archive",
                    safe_filename,
                    safe_sid,
                )
        except docker.errors.NotFound:
    ...
    def upload_files_batch(
        ...
        try:
            if RCE_DATA_DIR_HOST:
                ...
                logger.info(
                    "Uploaded %d files to volume for session %s", len(files), safe_sid
                )
                self.get_or_create_container(
                    safe_sid, external_session_id=external_session_id
                )
                ...
                logger.info(
                    "Uploaded %d files to session %s via put_archive",
                    len(files),
                    safe_sid,
                )
        except docker.errors.NotFound:
    ...
    def get_file_id_mapping(
        self, nanoid_session: str, filenames: List[str]
    ) -> Dict[str, str]:
        """
    ...
    def download_file(self, session_id: str, filename: str):
        ...
        if RCE_DATA_DIR_HOST:
            ...
            try:
                ...
                logger.error(
                    "Failed to download file %s from session %s: %s",
                    filename,
                    session_id,
                    e,
                )
                raise HTTPException(
                    status_code=500, detail="Internal server error during file download"
                )

    def list_files(self, session_id: str, external_session_id: Optional[str] = None):
        container = self.get_or_create_container(
            session_id, external_session_id=external_session_id
        )
        ...
        try:
            ...
            container = self.get_or_create_container(
                session_id, force_refresh=True, external_session_id=external_session_id
            )
            res = container.exec_run(cmd=cmd, demux=True)
        ...
        if isinstance(res.output, tuple) and len(res.output) == 2:
            ...
            stdout = (
                res.output
                if isinstance(res.output, bytes)
                else res.output.encode("utf-8")
            )
            stderr = None
        else:
        ...
        logger.info(
            "list_files exec_run for session %s: exit_code=%s, stdout='%s', stderr='%s'",
            session_id,
            res.exit_code,
            out_str.strip(),
            err_str.strip(),
        ...
        if res.exit_code == 0:
            ...
            try:
                ...
                logger.error(
                    "Failed to parse file list JSON from container: %s. Raw output: %s",
                    e,
                    output,
                )
    ...
    def _execute_in_container(
        ...
        return container.exec_run(cmd=cmd, workdir="/mnt/data", demux=True)
        ...
        try:
            ...
            try:
                exec_result = self._execute_in_container(
                    container, wrapped_code, container_path, code_filename, lang
                )
            except (docker.errors.APIError, docker.errors.NotFound):
            ...
            raise HTTPException(
                status_code=500,
                detail="An internal error occurred during code execution.",
            )
        finally:
            ...
            if container and container_path:
                try:
                    ...
                    logger.warning(
                        "Failed to remove temporary file %s in session %s",
                        container_path,
                        session_id,
                    )
...
def _get_effective_session_id(req: CodeRequest) -> Optional[str]:
    ...
    if not effective_session_id and req.files and len(req.files) > 0:
        ...
        elif hasattr(first_file, "storage_session_id") and getattr(
            first_file, "storage_session_id"
        ):
...
def _get_validated_lang(req: CodeRequest) -> str:
    ...
    if requested_lang not in SUPPORTED_LANGUAGES:
        ...
        raise HTTPException(
            status_code=400,
            detail=f"Unsupported language: {req.lang}. Supported: python, bash, r",
        )
...
async def run_code(req: CodeRequest, key: str = Security(get_api_key)):
        ...
        for file_info in req.files:
            ...
            if file_sid:
                ...
                if file_real_sid != real_session_id:
                    try:
                        logger.info(
                            "ファイルをセッション %s から実行セッション %s に集約します: %s",
                            file_nanoid_sid,
                            nanoid_session,
                            file_info.name,
                        )
                        file_content, _ = await asyncio.to_thread(
                            kernel_manager.download_file, file_real_sid, file_info.name
                        )
    ...
    current_files = await asyncio.to_thread(
        kernel_manager.list_files, real_session_id, external_session_id=nanoid_session
    )
    logger.info(
        "Found %d files in session %s (real: %s): %s",
        len(current_files),
        nanoid_session,
        real_session_id,
        current_files,
    ...
    if req.files:
        for f_in in req.files:
            name = getattr(f_in, "name", None) or (
                f_in.get("name") if isinstance(f_in, dict) else None
            )
            if name:
    ...
    for f in current_files:
        ...
        if not is_inherited and mime_type.startswith("image/"):
            try:
                img_bytes, _ = await asyncio.to_thread(
                    kernel_manager.download_file, real_session_id, f
                )
                ...
                logger.info(
                    "Captured image %s (bytes: %d, format: %s)", f, len(img_bytes), ext
                )
            except Exception as e:
                logger.warning(
                    "Failed to encode generated image %s for response: %s", f, e
                )
...
async def upload_files(
    ...
    try:
        ...
        await asyncio.to_thread(
            kernel_manager.upload_files_batch,
            real_session_id,
            file_data,
            external_session_id=nanoid_session,
        )
        ...
        uploaded_files = [
            {"fileId": filename_to_id[name], "filename": name} for name in filenames
        ]
...
async def list_session_files(session_id: str, key: str = Security(get_api_key)):
    ...
    try:
        ...
        with kernel_manager.lock:
            nanoid_session = kernel_manager.session_to_nanoid.get(
                real_session_id, s_sid
            )

        files = await asyncio.to_thread(
            kernel_manager.list_files,
            real_session_id,
            external_session_id=nanoid_session,
        )
...
async def download_session_file(
    ...
    """
    real_session_id, real_filename = kernel_manager.resolve_download_ids(
        session_id, filename
    )
    ...
    try:
        if RCE_DATA_DIR_HOST:
            ...
            in_memory_content, mtime = await asyncio.to_thread(
                kernel_manager.download_file, real_session_id, real_filename
            )
    except HTTPException:
        raise
    except Exception as e:
        logger.error(
            "Failed to download file %s from session %s: %s", filename, session_id, e
        )
        raise HTTPException(
            status_code=500, detail="Internal server error during file download"
        )
    ...
    if in_memory_content is not None:
        return Response(
            content=in_memory_content, media_type=mime_type, headers=headers
        )

    return FileResponse(path=tmp_filepath, media_type=mime_type, headers=headers)
