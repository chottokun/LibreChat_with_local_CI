...
def _create_last_res_assign(value: ast.expr) -> ast.Assign:
    """Creates the AST node for: __last_res__ = <value>"""
    return ast.Assign(
...
def _create_last_res_if_print() -> ast.If:
    """Creates the AST node for: if __last_res__ is not None: print(repr(__last_res__))"""
    return ast.If(
        test=ast.Compare(
...
class KernelManager:
    ...
    def resolve_download_ids(self, session_id: str, filename: str) -> Tuple[str, str]:
        ...
        with self.lock:
            ...
            if (
                nanoid_session in self.file_id_map
                and s_filename in self.file_id_map[nanoid_session]
            ):
    ...
    def start_new_container_unlocked(
        self, session_id: str, external_session_id: Optional[str] = None
    ):
    ...
    def list_files(self, session_id: str, external_session_id: Optional[str] = None):
        ...
        cmd = [
            "python3",
            "-c",
            "import os, json; print(json.dumps(os.listdir('/mnt/data')))",
        ]
...
async def run_code(req: CodeRequest, key: str = Security(get_api_key)):
    ...
    if not effective_session_id:
        ...
        if LAST_UPLOADED_SESSION_ID and (time.time() - LAST_UPLOAD_TIME < 300):
            ...
            logger.info(
                "Fallback activated! Re-using last uploaded session ID: %s",
    ...
    with kernel_manager.lock:
        ...
        existing_filenames_to_ids = {
            v: k for k, v in kernel_manager.file_id_map[nanoid_session].items()
        }
...
async def upload_files(
    ...
    try:
        ...
        if isinstance(kernel_manager, MagicMock):
            ...
            real_session_id, nanoid_session = (
                kernel_manager.get_or_create_session_mapping(sid)
            )
        ...
        res = {"message": "success", "session_id": sid, "files": uploaded_files}
...
def get_download_meta(real_filename: str) -> Tuple[str, Dict[str, str]]:
    ...
    safe_filename_ascii = (
        real_filename.encode("ascii", "ignore")
        .decode()
        .replace("\\", "")
        .replace('"', "")
        .replace("\r", "")
