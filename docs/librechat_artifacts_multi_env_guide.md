...
graph TD
    ...
    subgraph 起動プロファイルによる分岐
        ...
        F[環境: SSL検証・オンプレ本番 / フェーズ2,3] -->|--profile ssl-mode を指定| G[LibreChat本体 + RCE + Sandpack Bundler起動]
    end
...
- **フェーズ2（移行期）**: Caddyによるローカル自己署名証明書を用いた仮HTTPSテスト環境の構築。マルチバインドによるSNI検証とブラウザでの例外許可・CA登録を行い、Artifactsのレンダリング・動作検証を実施。
- **フェーズ3（オンプレミス本番）**: 組織内のプライベートCA（社内CA）が発行した正規SSL証明書の適用。社内DNSによるドメイン名解決と証明書の組織内一括配布により、社内クライアントから警告なしでセキュアにフル機能（Artifacts）を利用。
...
### 4.3 オンプレミス本番環境（フェーズ3）に向けた社内CA証明書の適用計画

フェーズ3（オンプレミス本番）では、自己署名証明書（`tls internal`）から、**組織内のプライベートCA（認証局）が発行した正規のSSL/TLS証明書**へとシームレスに切り替えます。

本コードベースでは、証明書ファイルが存在する場合はそれを使い、存在しない場合は自動的に自己署名（`internal`）にフォールバックする**ハイブリッドTLS設計**がすでにインフラ定義レベルで組み込まれています。

#### 1. 証明書の格納とマウント
社内CAから取得したサーバー証明書（例: `librechat.crt`）と秘密鍵（例: `librechat.key`）を、プロジェクト直下の `./certs` ディレクトリに配置します。このディレクトリは、Caddyコンテナ内の `/certs` に読み取り専用で自動マウントされます。

#### 2. 環境変数（.env）の指定
`.env` ファイルに、Caddyコンテナ内から見た証明書のパスを指定します。
```env
CUSTOM_CERT_PATH=/certs/librechat.crt
CUSTOM_KEY_PATH=/certs/librechat.key
```

#### 3. Caddyによる自動適用
Caddyは起動時、指定された環境変数を展開し、自動的に組織内CA発行の証明書をバインドします。クライアントPC群にはすでに社内CAのルート証明書がインポートされているため、各ブラウザから警告なしで完全にセキュアにアクセス可能となります。
