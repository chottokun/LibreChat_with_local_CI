...
#### 5. ボリュームマウント時のセッション間ファイル隔離
* `RCE_DATA_DIR`（ボリュームマウントモード）利用時、ホスト側のルート保存領域全体をコンテナに渡すのではなく、**セッション単位の個別サブディレクトリ（例: `<RCE_DATA_DIR>/<session_id>`）のみをピンポイントでコンテナ内の `/mnt/data` にバインド**します。
* これにより、コンテナ内のユーザー（UID: 1000）からは他ユーザーや他セッションのディレクトリを閲覧・改ざんすることは物理的に不可能です。
* さらに、API層のパス検証（`is_relative_to`）および Nanoid サニタイズにより、シンボリックリンクや相対パスを用いたディレクトリ脱出（Path Traversal）も完全に遮断されます。
...
#### 2. アイドルセッション自動クリーンアップと再生成ライフサイクル
* **軽量コンテナ設計**: サンドボックスイメージ（約700MB）は全コンテナで読み取り専用共有され、起動中コンテナの差分書き込みレイヤーは数十KB〜数MB程度です。また、待機中プロセスのメモリ消費もごく僅かであるため、**`RCE_SESSION_TTL` を数時間〜24時間（例: 86400秒）などの長期間に設定してもホストのリソースを圧迫しません**。
* **自動クリーンアップ (`cleanup_loop`)**: バックグラウンドタスクが定期的に `RCE_SESSION_TTL` を超過したアイドルセッションを検出し、コンテナ停止およびクリーンアップを実行します。
* **オンデマンド自動再生成**: TTL 経過によってコンテナが破棄された後でも、同一セッション（同じチャット会話）でユーザーが再度コード実行を要求した場合は、API が即座に**新しいクリーンなサンドボックスコンテナを自動生成**して実行を継続します。
