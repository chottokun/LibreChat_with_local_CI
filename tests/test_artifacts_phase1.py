...
def test_env_librechat_sandpack_bundler_url_phase1():
    ...
    assert "【フェーズ2: Nginx SSLテスト環境 (自己署名証明書)】" in content, "フェーズ2の説明が .env.librechat に含まれていません。"
    assert "【フェーズ3: オンプレミス本番 / 正式なCA証明書の適用】" in content, "フェーズ3の説明が .env.librechat に含まれていません。"
