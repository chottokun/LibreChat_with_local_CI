...
AIエージェントは、新機能の実装やバグ修正を行う際、**テストファースト（TDD）で網羅的なテストコードを同時に構築**しなければなりません。

### 2.1 必須のテスト検証項目
特に以下の重要領域を変更する際は、必ず `tests/` 内に該当するテストケースが存在、または合格することを確認してください：
...
### 4.4 サンドボックスコンテナのネットワーク無効化 (デフォルト)
