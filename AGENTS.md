...
### 5.1 セッションIDの自動解決（チャット分離保証）
`main.py` の `_get_effective_session_id` は以下の優先順でセッションIDを特定します。このフォールバック順序を変更・削除しないでください。
1. リクエストの `session_id` または `entity_id`
2. `files` 配下に含まれる `session_id` もしくは `storage_session_id`
3. 欠落時は常に新規セッションID (Nanoid) を生成

**禁止事項**: グローバル変数 `LAST_UPLOADED_SESSION_ID` や `user_{user_id}` などの共有フォールバックロジックを `/exec` および `/upload` エンドポイントのセッション解決に使用してはなりません。これらはチャット間・ユーザ間のセッション汚染を引き起こすため完全廃止されています。
